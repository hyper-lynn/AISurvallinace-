...
// 断开所有长链接
C_API void lcopenapi_client_clear_all();
...
// 成功返回0，失败返回-1
C_API int lcopenapi_client_request(LCOpenApiClient hc, LCOpenApiRequest *req, LCOpenApiResponse *resp, int timeout,int* sslCost, int* apiCost, int* reuseType);

C_API int lcopenapi_client_request_with_uri(LCOpenApiClient hc,const char *uri,const char *body, LCOpenApiResponse *resp, int timeout, int isUseAlive,int* sslCost, int* apiCost, int* reuseType);
...
#define LCOPENAPI_DESTROY_OBJECT(ptr)  { if((ptr)->destroy) (ptr)->destroy((ptr)); }
