#include "SearchDeviceBar.h"

SearchDeviceBar::SearchDeviceBar(QWidget *parent)
	: QWidget(parent)
{
	m_textEdit = NULL;
	m_searchBtn = NULL;
...
SearchDeviceBar::~SearchDeviceBar()
{
	if (m_textEdit)
	{
		delete m_textEdit;
...
int SearchDeviceBar::init()
{
	...
	m_textEdit->setPlaceholderText(tr("--Please Input DeviceID--"));
	...
	connect(m_searchBtn, SIGNAL(clicked()), this, SLOT(slotSearchDevice()));
...
void SearchDeviceBar::setDeviceID(QString deviceID)
{
	m_textEdit->setText(deviceID);
}

void SearchDeviceBar::slotSearchDevice()
{
	QString deviceID = m_textEdit->text();
	printf("==SearchDeviceBar::slotSearchDevice deviceID=%s\n", deviceID.toStdString().c_str());
	emit sgnSearchDevice(deviceID);
	//清空输入的内容
	m_textEdit->clear();
}

bool SearchDeviceBar::eventFilter(QObject *obj, QEvent *event)
{
...
QString SearchDeviceBar::getDeviceID()
{
