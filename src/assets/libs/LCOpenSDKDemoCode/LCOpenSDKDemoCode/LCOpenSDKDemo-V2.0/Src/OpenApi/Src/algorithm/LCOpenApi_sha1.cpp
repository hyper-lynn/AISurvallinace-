...
int common_algorithm_sha1(char *data, int len, char *outbuf)
{
	...
	if (!data || len <= 0 || !outbuf)
	{
		return -1;
	}

	sha1_init(&ctx);
	sha1_write(&ctx, (unsigned char *)data, (size_t)len);
	sha1_final(&ctx);

	memcpy(outbuf, ctx.buf, 20);
...
int common_algorithm_hmacsha1(char *data, int len, char *key, int key_len, char *outbuf)
{
	...
	{
		sha1_init(&ctx);
		sha1_write(&ctx, (unsigned char *)key, (size_t)key_len);
		sha1_final(&ctx);
		memcpy(tk, ctx.buf, 20);
	...
	sha1_init(&ctx);
	sha1_write(&ctx, k_ipad, 64);
	sha1_write(&ctx, (unsigned char *)data, (size_t)len);
	sha1_final(&ctx);
	memcpy(outbuf, ctx.buf, 20);

	sha1_init(&ctx);
	sha1_write(&ctx, k_opad, 64);
	sha1_write(&ctx, (unsigned char *)outbuf, 20);
	sha1_final(&ctx);
	memcpy(outbuf, ctx.buf, 20);
