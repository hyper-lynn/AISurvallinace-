...
static int build(LCOpenApi_LCOpenApiRequest *p)
{
	...
	const char* str = "T";
	p->body = lcopenapi_request_build(p, &req->data, METAINFO(AccessTokenRequestData), const_cast<char*>(str));
	if (p->body.cstr == NULL)
	{
		return -1;
	}
...
AccessTokenRequest *LCOpenApi_LCOPENAPI_INIT(AccessTokenRequest)
{
	...
	if (METAINFO(AccessTokenRequestData) == NULL)
	{
		...
		METAINFO_ADD_MEMBER(AccessTokenRequestData,AccessTokenRequest, FIELD_TYPE_INT, _nouse);
