    ...
    , m_endTimeText(NULL)
{
    if (info)
    {
/* this holds info, info is responsible for thumbnail download and display */
        info->view = this;
...
void RecordThumbView::init()
{
    do 
    {
        ...
        m_beginTimeText->setText("Record begin time:" + m_info.beginTime);
        ...
        m_endTimeText->setText("Record end time:" + m_info.endTime);
...
void RecordThumbView::fillBackGroundWithColor(QWidget *widget, const QColor& color)
{
    if (NULL == widget)
    {
        return;
...
/* easy4ip get record thumbnail interface */
            if (!g_isReleasedForOverseas)
            {
                m_progressBar->show();
