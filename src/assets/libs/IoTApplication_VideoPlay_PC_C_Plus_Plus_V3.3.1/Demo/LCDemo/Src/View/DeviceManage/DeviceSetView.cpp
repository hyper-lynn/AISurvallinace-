...
int DeviceSetView::init()
{
    ...
    do 
    {
        ...
        m_ReturnToDevicesListBtn->setBtnAttribute("Skin/common_icon_back", "Back to Devices List", 11, QColor(0xb0, 0xaf, 0xaf));
...
void DeviceSetView::show()
{
    ...
    {
        int alarmStatus = 0;
        QString sRet = OpenApiCtrl::getInstance()->bindDeviceInfo(channelInfo->deviceId, channelInfo->channelId, alarmStatus);
        if ("OK" != sRet)
        {
            m_statusLbl->setText(tr("Status>>>Get device alarm status failed") + "[" + sRet + "]");
        }
        channelInfo->alarmStatus = alarmStatus;
        setAlarmStatusBtnPixmap((1 == alarmStatus) ? true : false);
        setCloudStorageStatusBtnPixmap((1 == channelInfo->csStatus) ? true : false);
    }
...
void DeviceSetView::slotChangeAlarmStatus()
{
    ...
    if (NULL != channelInfo)
    {
        bool alarmStatusFlag = (1 == channelInfo->alarmStatus) ? false : true;
        QString ret = m_OpenApiCtrlInstance->modifyDeviceAlarmStatus(channelInfo->deviceId, channelInfo->channelId, alarmStatusFlag);
        if ("OK" == ret)
        {
            channelInfo->alarmStatus = alarmStatusFlag ? 1 : 0;
...
void DeviceSetView::slotChangeCloudStorageStatus()
{
    ...
    if (NULL != channelInfo)
    {
        bool cloudStorageStatusFlag = (1 == channelInfo->csStatus) ? false : true;
