...
int DeviceAddView::init()
{
    ...
    do 
    {
        ...
        m_DeviceIdLbl->setFixedHeight(13);
...
void DeviceAddView::slotAddDevice()
{
    ...
    iRet = LCOpenSDK_DeviceInit::getInstance()->searchDeviceInitInfo(m_deviceId.toLatin1().data(), m_devInitInfo, 10000);
    if (-2 == iRet)
    {
...
void DeviceAddView::slotDecryptKeyBoxOK(QString deviceKey)
{
    ...
    if (m_devInitInfo.initStatus == 1)
    {
		/* 不需要检测密码是否正确，直接初始化 */
	    iRet = LCOpenSDK_DeviceInit::getInstance()->initDevice(m_devInitInfo.szMac, deviceKey.toLatin1().data(), m_devInitInfo.szIp);
		if (-2 == iRet)
		{
    ...
    else if((m_devInitInfo.initStatus == 0 || m_devInitInfo.initStatus == 2) && g_isReleasedForOverseas)
    {
...
void DeviceAddView::restApiBind(QString deviceKey)
{
	...
	{
		QString ability;
		sRet = m_OpenApiCtrlInstance->unBindDeviceInfo(m_deviceId,ability);
		if ("OK" == sRet)
		{   
