...
#include "View/RecordList/RecordThumbView.h"
...
void RecordListScrollView::refresh(QString decryptKey)
{
    ...
    if (m_recordChildList.size() > 0)
    {
        ...
        m_CurlDownload->get(m_recordChildList[m_downloadIndex]->thumbUrl);
    }
}

void RecordListScrollView::slotCurlDownloadFinished(unsigned char* data, unsigned int dataLen)
{
    ...
    if (NULL != data && dataLen > 0)
    {
        ...
        int ret = LCOpenSdk_Utils::DecryptPic((const char*)data, dataLen, m_decryptKey.toLocal8Bit().data(), devID, devPwd, (char*)m_decryptPicBuf, decryptPicBufLen);
        if (0 == ret)
        {
            int pngImgSize = PNG_IMG_MAX_SIZE;
            /*if (LCOpenSdk_Utils::Jpg2Png(m_decryptPicBuf, decryptPicBufLen, m_pngImgData, pngImgSize))
            {
                QPixmap pixmap;
