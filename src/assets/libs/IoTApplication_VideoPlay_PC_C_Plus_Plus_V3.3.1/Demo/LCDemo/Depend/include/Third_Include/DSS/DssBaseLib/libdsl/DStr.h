...
public:  // 一些静态函数，封装基础功能，进行安全增强
	...
	// 增强的strncpy，保证不越界且以'\0'结尾，最多只复制max_copy_len个字节,如果dst空间不足以容纳max_copy_len个字节，则最多拷贝dst_len-1个字节
	static char *strncpy_x(char *dst, int dst_len, const char *src, int max_copy_len);
