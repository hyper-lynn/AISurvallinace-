...
// <4> 从Trunk上合并来，为了保证兼容 将其重命名为DRefPoolESB; TODO:后续将其与DRefPool合并

template< class T >
class DRefPoolESB : virtual public DRefObj
{
public:
	DRefPoolESB( unsigned int nObjectMaxCount = DSL_OBJECT_POOL_MAX_NUM ) : m_nObjectMaxCount( nObjectMaxCount ) 
	{ 
		m_dequeObjects = new DRefObj*[nObjectMaxCount];
		m_nObjectCount = 0;
	}
	virtual ~DRefPoolESB(){ Decrease( 0 ); }
...
public:
	...
	inline DRef<T> GetObject()
	{
		...
		if(pT)
			pT->setpool( (DRefPoolESB< DRefObj > *)this, (DRefObj*)this );
		return DRef<T>( pT );
	}
...
private:
	...
	inline void recycle( DRefObj * pObj )
	{
		...
		m_mtxObj.Lock();
		if(m_nObjectCount >= m_nObjectMaxCount) 
		{
			// 引用计数已经为0，setpool(NULL)在destroy相当于delete，
			for(; m_nObjectCount > m_nObjectMaxCount / 2; )
