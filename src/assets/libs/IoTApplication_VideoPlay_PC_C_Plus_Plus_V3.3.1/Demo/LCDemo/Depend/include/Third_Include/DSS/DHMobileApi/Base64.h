...
/// 计算存放编码后数据的缓冲区字节数
INTERFACE_API int base64EncodeLen(int len);
...
INTERFACE_API int base64Encode(char* dst, const char* src, int len);

/// 计算存放解码后数据的缓冲区字节数
INTERFACE_API int base64DecodeLen(const char* src);

/// Base64 解码
/// \param [in] dst 解码前的文本缓存
/// \param [in] src 解码后的数据缓存
/// \return 返回解码后的数据字节数
INTERFACE_API int base64Decode(char* dst, const char* src);
