//
//  "$Id: StreamPackage.h 785019 2019-09-05 05:56:44Z 12012 $"
//
...
public:
	...
	public:
		/// 创建实时流封装器
		/// \param [in] packetType 封装类型，支持："FLV","ASF","DAV","RAW"
		/// \retval 非NULL 创建IStreamPackage类对象指针
		/// \retval NULL 创建失败
		// 注：	当类型为"RAW"时，表示对裸数据进行加密，是恒信加密码流模块
	...
	// 按帧输入裸数据，以及帧信息
	/// \param [in] framePacket 用CPacket的形式给一帧裸数据
	/// \param [in] inFrameInfo 给的裸数据的信息
	/// \param [in] extPacket 传入的CPacket，相当于一段内存，将来封装数据会拷贝到这个CPacket中
	/// \param [in] outFrameInfo 封装之后的帧信息
	/// \retval true 成功
	/// \retval false 失败
	virtual bool inputFrameEx(std::deque<Dahua::Memory::CPacket> &framePacket, PackageCmptInfo const& inFrameInfo, Memory::CPacket& extPacket, PackageCmptInfo & outFrameInfo)
		COMPONENT_UNIMPLEMENTED_OPERATION;
	...
	/// 用于设置扩展信息
	/// param [In] type 扩展信息类型，"encrypt"或"ENCRYPT"为加密
	/// 当类型为加密的时候，会调用基类的SetEncryptType进行加密，目前只支持AES加密
	/// param [In] pExtInfo
	/// param [In] nLength
	/// \retval true 成功
	/// \retval false 失败
	...
	/// 获取封装格式建议的编码参数集
	/// \param [in] type 编码参数集类型，取值capacityType
	/// \param [out] capacity 建议的编码参数集，存储在结构体VideoConstraint和AudioConstraint中
	/// \param [out] capacity_count 建议的编码参数集数目
	/// \retval true 成功
	/// \retval false 失败
	virtual bool getPacketCapacity(CapacityType type, const void** capacity, int* capacity_count) const
		COMPONENT_UNIMPLEMENTED_OPERATION;
