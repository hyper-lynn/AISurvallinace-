...
#define DEFAULT_RECV_BUFFER		1500	// 接收缓冲默认大小
#define MAX_RECV_BUFFER			(128 * 1024)	// 接收缓冲最大值
...
public:
	...
	virtual SOCKID CreateSocket( const DRef<DSocketHandler> & pEvent, int * error = NULL );
	virtual int CloseSocket(SOCKID hSocket);
	
	virtual int StartTcpServer( SOCKID hSocket, const char * localAddr, int localPort, int backlog = 128 );
	virtual int StartTcpClient( SOCKID hSocket, const char * remoteAddr, int remotePort, int timeout_ms = DEFAULT_CONNECT_TIMEOUT_MS );
	virtual int StartUDP( SOCKID hSocket, const char * localAddr, int localPort, const char * remoteAddr = NULL, int remotePort = 0, bool bIgnoreUnreachable = true);
	...
	// 设置接收数据缓冲区大小
	int SetRecvBufSize( SOCKID hSocket, int max_size );
	...
	virtual int Send( SOCKID hSocket, const char * data, int dataLen ); // 支持TCP和UDP；对于UDP，发到创建时提供的remoteAddr中
	virtual int SendTo( SOCKID hSocket, const char * data, int dataLen, const char * remoteAddr, int remotePort ); // udpSocket向多个不同地址发时的专用接口

	// 切换Socket的回调函数
	virtual int SetHandler(SOCKID hSocket, DSocketHandler * handler);

	// 是否端口复用，需要在StartTcpServer或StartUDP之前调用
	// 0:禁止; 1:允许; -1:由基础库决定，默认-1
	virtual int SetReuseAddr(SOCKID hSocket, int reuse);

	// 需要在StartUDP之前调用，默认为0.0.0.0
	virtual int SetMulticastInterface(SOCKID hSocket, const char* localAddr);
