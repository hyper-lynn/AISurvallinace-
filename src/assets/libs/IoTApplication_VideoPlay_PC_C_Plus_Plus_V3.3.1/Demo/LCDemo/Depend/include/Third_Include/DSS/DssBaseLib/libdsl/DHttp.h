...
enum {
	...
	DHTTP_AUTH_DIGEST_AUTHEN,		// 服务端发送给客户端的Digest鉴别回应
	DHTTP_AUTH_DIGEST_AUTHOR,		// 客户端发送给服务端的Digest认证请求
	DHTTP_AUTH_DIGEST_AUTHEN_INFO,	// 服务端发送给客户端的Digest鉴别信息
};
...
public:
	...
	inline int SetDigestAuthenInfo( const DStr & ha1, const DStr & nc = DStr(), const DStr & nextnonce  = DStr() )
	{
		return SetDigestAuthenInfo( ha1.c_str(), nc.c_str(), nextnonce.c_str() );
	}
	int SetDigestAuthenInfo( const char * ha1, const char * nc = NULL, const char * nextnonce = NULL );
