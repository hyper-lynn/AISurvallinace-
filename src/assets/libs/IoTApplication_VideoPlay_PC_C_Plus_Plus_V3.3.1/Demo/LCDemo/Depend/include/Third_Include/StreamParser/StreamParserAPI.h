...
#ifndef __INCLUDED_DAHUA_MEDIAPARSER_STREAMPARSERAPI_H__
#define __INCLUDED_DAHUA_MEDIAPARSER_STREAMPARSERAPI_H__
...
enum SP_RESULT
{
    ...
    SP_ERROR_FILE_TYPE_NOSUPPORT,       /*文件解析方式不支持*/
    SP_ERROR_STREAM_NOSUPPORT,          /*流解析方式不支持*/
...
enum
{
	...
	SP_FRAME_SUB_TYPE_POPULATION_PIC_OVERLAY,		/*人群密度叠加图片*/
	SP_FRAME_SUB_TYPE_DATA_OSD,						/*叠加OSD帧*/
	SP_FRAME_SUB_TYPE_SCENE_INFORMATION_TAGGING,	/*景物点信息标注*/
	SP_FRAME_SUB_TYPE_DATA_WITH_LARGE_AMOUNT,		/*数据帧为智能大数据量帧，守望者人群密度图帧*/
	SP_FRAME_SUB_TYPE_DATA_CIPHER_AUXILIARY_FRAME,	/*数据为密码辅助帧*/
	SP_FRAME_SUB_TYPE_DATA_WITH_WATER_LEVEL_MONITOR,/*数据帧为水位检测水位尺信息帧*/
	SP_FRAME_SUB_TYPE_DATA_WITH_SOUND_DECIBEL,		/*数据帧为声音分贝值*/
	SP_FRAME_SUB_TYPE_DATA_WITH_SMART_MOTION,		/*数据帧为智能动检帧*/
...
enum SP_ENCODE_VIDEO_TYPE
{
	...
	SP_ENCODE_VIDEO_SVAC2,				/*视频编码格式是SVAC2.0*/
...
enum SP_ENCODE_AUDIO_TYPE
{
	...
	SP_ENCODE_AUDIO_G722,				/*音频编码格式是G722*/
	SP_ENCODE_AUDIO_G722_1C_48,			/*音频编码格式是G722_1C_48*/
	SP_ENCODE_AUDIO_AAC_LD,				/*音频编码格式是AAC_LD*/
	SP_ENCODE_AUDIO_OPUS,				/*音频编码格式是OPUS*/
	SP_ENCODE_AUDIO_G719,				/*音频编码格式是G719*/
	SP_ENCODE_AUDIO_G728,				/*音频编码格式是G728*/
	SP_ENCODE_AUDIO_G722_1_16,			/*音频编码格式是G722_1_16*/
	SP_ENCODE_AUDIO_G722_1_24,			/*音频编码格式是G722_1_24*/
	SP_ENCODE_AUDIO_G722_1C_24,			/*音频编码格式是G722_1C_24*/
	SP_ENCODE_AUDIO_G722_1C_32,			/*音频编码格式是G722_1C_32*/
...
enum
{
	...
	SP_STREAM_TYPE_ZSLC_PS,		//中视里程PS
	SP_STREAM_TYPE_STAR_EX,		//一种新的星望私有流
	SP_STREAM_TYPE_DONGYANG,	//东阳的一个特殊码流
	SP_STREAM_TYPE_CREARO,		//创世码流
	SP_STREAM_TYPE_MP3,			//MP3纯音频格式
	SP_STREAM_TYPE_AAC_ADTS,	//AAC的ADTS纯音频格式
	SP_STREAM_TYPE_XGZT,		//香港智腾
	SP_STREAM_TYPE_HBStreamV30,	//汉邦码流
};
...
enum
{
	...
	SP_ERROR_FLAGS_SUBTYPE_UNKNOWN,		/*未知帧类型*/
	SP_ERROR_FLAGS_DECRYPTION_FAILURE,	/*解密失败*/
};
...
enum
{
	...
	SP_ENCRYPT_AES256,						/*AES256-OFB-NOPADDING*/
	SP_ENCRYPT_AES256_GDPR2,                /*AES256 GDPR二期加密算法*/
	SP_ENCRYPT_SM1_ECB,                     /*国密加密算法,SM1算法 ECB模式 NoPadding*/       
	SP_ENCRYPT_SM1_OFB,                     /*国密加密算法,SM1算法 OFB模式 NoPadding*/ 
	SP_ENCRYPT_SM4_ECB,                     /*国密加密算法,SM4算法 ECB模式 NoPadding*/ 
	SP_ENCRYPT_SM4_OFB,                     /*国密加密算法,SM4算法 OFB模式 NoPadding*/ 
};
...
enum
{
	SP_POPULATION_OVERLAY_PIC_PURPOSE_POPULATION_DENSITY_INCREASED,	/*叠加图片用途：人群密度集增加*/
};

/*叠加图片动作*/
enum
{
	SP_POPULATION_OVERLAY_PIC_ACTION_STACKING_END,			/*叠加图片动作：停止叠加*/
	SP_POPULATION_OVERLAY_PIC_ACTION_STACKING_START,		/*叠加图片动作：开始叠加*/
};

/*叠加图片编码格式*/
enum
{
	SP_POPULATION_OVERLAY_PIC_CODEC_FORAMAT_JPEG,			/*叠加图片编码格式为JPEG*/
};

//OSD字符对齐方式
enum
{
	SP_OSD_DATA_ALIGN_LEFT,			/*左对齐*/
	SP_OSD_DATA_ALIGN_RIGHT,		/*右对齐*/
};

//OSD字体
enum
{
	SP_OSD_DATA_SIMSUN,			/*宋体*/
};
...
typedef struct
{
	char* pKey;              //AES:key；AES256(GDPR一期):key； AES256(GDPR二期)和国密:vkek   			   
	int nKeyLen;			
	char* pKeyId;       	    //AES:不用填； AES256(GDPR一期):keyid； AES256(GDPR二期)和国密:vkekid 		 
	int nKeyIdLen;	
	bool bSetPublicKey;         //是否设置公钥  pPublicKey
	SP_PUBLICKEY_PARAM* pPublicKey;     //svac2.0 国密解签名需要填。其余都填NULL
	char nReserved[19];      //保留字段      
}SP_DECRYPT_PARAM; 
...
typedef struct
{
	...
	unsigned char incompleteFrameFlag;	//标识不完整帧
	...
	unsigned char nReserved2[1];

	unsigned short nOsdTopLeftCornercoordinateX; //字符叠加区域左上角x坐标
	unsigned short nOsdTopLeftCornercoordinateY; //字符叠加区域左上角y坐标

	unsigned char  nOsdWordSize;				//字号
	unsigned char  nOsdWordAlignment;			//字符对齐方式

	unsigned short nB5HeadEncryptCrc16;			// 0xB5加密帧CRC16结果
	unsigned char  nB5HeadIV[16];				// 0xB5加密IV
	unsigned char  nB5HeadEncryptVKid[16];		// 0xB5加密VKID
	unsigned int   nOsdRgbaValue;				//字符颜色
	
	unsigned char nB6HeadGmIv[16];         //国密-解密所需IV
	unsigned int nB6HeadVekEncryptType;    //国密-加密VEK所用的加密类型
	unsigned char nB6HeadVkekVersion[20];  //国密-VKEK版本号
	unsigned char nB6HeadVek[16];          //国密-VEK

	int             nVideoBitDepth;        //视频位深，如果值为0，则默认位深为8
	unsigned char nReserved[164];
...
typedef	struct
{
	...
	SPint64		index;		// 帧回调顺序，即第几帧，从0开始。负值为无效值
	/*待扩展*/
...
/********************************************************************
*	Funcname: 	    	SP_GetVersion
*	Purpose:	        获取svn版本信息
*   InputParam:         无
*   OutputParam:		无
*   Return:             库版本号字符串
*********************************************************************/
STREAMPARSER_API char* CALLMETHOD SP_GetVersion(void);
 ...
 *	Purpose:	        获取码流类型，在SP_ParseData之后调用
 ...
 *	Purpose:			在已知码流类型之后，获取支持的解析类型
 *  InputParam:         streamType:	码流类型，已知或在SP_GetStreamType接口或者SP_GetFileStreamType接口调用成功之后调用
 ...
 *  InputParam:         handle:	通过SP_CreateFileParser或者SP_CreateFileHandle返回的句柄
 ...
 *	Purpose:	        设置实时流解析秘钥
 *  InputParam:         handle: 通过SP_CreateStreamParser返回的句柄。
 *						type : 秘钥类型 ：如SP_ENCRYPT_AES,SP_ENCRYPT_AES256
 *						key：秘钥数据
 *						keylen：秘钥长度
 *  OutputParam:        无
 *  Return:             0:调用成功
 ...
 *	Purpose:	        设置文件解析秘钥
 *  InputParam:         handle: 通过SP_CreateFileHandle返回的句柄。
 *						type : 秘钥类型 ：如SP_ENCRYPT_AES,SP_ENCRYPT_AES256
 *						key：秘钥数据
 *						keylen：秘钥长度
 *  OutputParam:        无
 ...
 *	Funcname: 	    	SP_Destroy
 *	Purpose:	        销毁码流分析器
 *  InputParam:         handle: 通过SP_CreateStreamParser或SP_CreateFileParser或SP_CreateFileHandle返回的句柄。
 *  OutputParam:        无
 *  Return:             0:调用成功
 *                      其他值：失败，通过SP_GetLastError获取错误码 见SP_RESULT
*********************************************************************/
 ...
 *	Purpose:	        获得码流分析库错误码
 *  InputParam:         handle: 通过SP_CreateStreamParser或SP_CreateFileHandle返回的句柄。
 *  OutputParam:        无
 *  Return:             错误码
*********************************************************************/
...
/********************************************************************
 *	Funcname: 	    	SP_CreateFileParserHandle
 *	Purpose:	        创建文件分析器,如需解密，调用此函数后，调用SP_FileEncryptKey
 *  InputParam:			nStreamType:指定解析类型,可填写SP_STREAM_TYPE_UNKNOWN
 *						file: 文件名
 *  OutputParam:        无
 *  Return:             NULL: 表示失败
 *                      其他: 文件解析器句柄
*********************************************************************/
STREAMPARSER_API void* CALLMETHOD SP_CreateFileHandle(IN int nStreamType, IN char* file);

/********************************************************************
 *	Funcname: 	    	SP_FileParser
 *	Purpose:	        进行文件分析，文件信息以回调方式给出
 *  InputParam:			handle:通过SP_CreateFileHandle返回的句柄
 *						fileIndex: 索引信息回调
 *						fileInfo:文件信息回调
 *						userData:用户信息
 ...
 *                      其他值：失败，通过SP_GetLastError获取错误码 见SP_RESULT
*********************************************************************/
STREAMPARSER_API SP_RESULT CALLMETHOD SP_FileParser(IN void* handle, IN fFileIndex fileIndex, IN fFileInfo fileInfo, IN void* userData);


/********************************************************************
 *	Funcname: 	    	SP_GetFileStreamType
 *	Purpose:			获取文件封装格式类型
 *  InputParam:			handle:通过SP_CreateFileHandle返回的句柄 
 *  OutputParam:        streamtype:	码流类型
 *  Return:             0:调用成功
 *                      其他值：失败，通过SP_GetLastError获取错误码 见SP_RESULT   
*********************************************************************/
STREAMPARSER_API SP_RESULT CALLMETHOD SP_GetFileStreamType(IN void* handle, OUT int* streamType);
...
#endif // __INCLUDED_DAHUA_MEDIAPARSER_STREAMPARSERAPI_H__
