 ...
 * @param[in] pUserData 用户数据
 */
typedef void (CALLBACK* fSegmentRecordDataCBFun)(LONG nPort, char* pSegmentFileName, unsigned int nFileSize, void* pUserData);
 ...
 */
typedef struct{
	unsigned int	x;				// 旋转点像素x位置，不旋转的情况为左上角点
	unsigned int	y;				// 旋转点像素y位置，不旋转的情况为左上角点
	...
	char*			szOsdData;		// osd数据，使用utf-8编码
	unsigned int	fontSize;		// 字体大小,最大144
	unsigned int	rotateAngle;	// 旋转角度，范围0-359度
} OsdDataInfo;

typedef struct{
	char*			fontPath;	// 字体文件位置
	unsigned int	osdCount;	// OSD叠加数量，最多32个
	OsdDataInfo*		osdList;	// OSD列表
}YUV_OSD_INFO_EX;
