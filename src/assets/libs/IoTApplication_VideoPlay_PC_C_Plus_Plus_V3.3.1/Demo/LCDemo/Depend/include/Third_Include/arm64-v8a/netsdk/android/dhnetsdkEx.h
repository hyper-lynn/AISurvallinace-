...
{
    DWORD							dwSize;										// 结构体大小
    DWORD                           dwIVSEvent;                                 // 智能分析事件类型,参考dhnetsdk.h中定义的智能分析事件宏.
                                                                                // 当前支持情况如下:
                                                                                // EVENT_IVS_FACERECOGNITION:       dwStructType为 1 时,对应结构体 DEV_EVENT_FACERECOGNITION_INFO_V1
    DWORD                           dwStructType;                               // 指定解析的类型,具体含义参考 dwIVSEvent 字段的说明

}NET_IN_SET_IVSEVENT_PARSE_INFO;


///@brief 指定智能事件解析所用的结构体 用于解决java大结构体new对象慢导致的问题.该接口全局有效,建议在SDK初始化前调用
